...
Here's a short post on why you should consider developing a segmented model when building credit risk scorecards to help optimize approval rates. While segmentation offers a variety of benefits, this post aims to offer a different perspective. 

I will be reusing code from a previous post. Feel free to navigate to [this post](/post/gains_table) for additional information. Note that sample datasets are available for download from [here](/download)
...
## Segments
Typically, credit risk scorecards would have segments like `known goods`, `known bads`, `ever chaged off` etc. For simplicity, I will use available `FICO grade` in the dataset to segment `known goods` and `known bads`. For the purposes of this post, let's focus on the `known goods` customers only (mostly because policy filters would remove the known bad customers). 
...
For now, let's simply keep borrowers who have a `FICO grade` of `"E", "F" & "G"` in the `known bads` bucket and rest in the `known goods` bucket.
...
1. One to the entire population
2. And one model to the `known goods`
...
Now that we have a model, let's assume an `expected bad rate` target of `2%`. That is to say that I can only approve those customers who I believe have an expected default propensity of `2%` or less. Now the question is if that were the case, given the existing model, what would be my expected `approval rate`? Note that I need to maximise my approval rate (to better utilise my applicant funnel).
...
```
## Simulations
To evaluate the through the door population better, let's run some simulations. Let's assume that only `known good` customers will be assessed through the risk scorecard. Let's write a small function to generate these simulations. 
...
```
When using the known-good model, we can achieve a slightly higher approval rate keeping the event rate more or less the same. While the difference is not significant, a ~1% higher approval rate on a large funnel could be significant. 
