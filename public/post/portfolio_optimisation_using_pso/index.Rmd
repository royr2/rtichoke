...
subtitle: ""
summary: "How to perform portfolio optimisation using particle swarm optimisation in R"
author: "royr2"
date: 2021-11-02
...
```{r echo = F, eval = F}
ticker_df <- na.omit(ticker_df)
write.csv(ticker_df, "ticker_database.csv")
```

```{r echo = F}
ticker_df <- read.csv("ticker_database.csv")
```
...
We can now visualise the function we were trying to optimise as well as the path each particle took to get to the optimal result. 
...
grid$obj <- apply(grid, 1, obj_func, ret_vec = mean_returns_small, cov_mat = cov_mat_small, 
                  lambda1 = 1, risk_av = 1000)

plot_ly(grid, x = ~x, y = ~y, z = ~obj) %>% 
  add_mesh()
```


```{r}
# Output has a data frame that stores particle locations and objective
# values at those locations
p <- plot_ly(out$X, x = ~X1, y = ~X2, z = ~obj) %>% 
  add_markers(color = ~iter, size = 1)

htmltools::tagList(list(p))
```

Looks a little odd but gets the point across (hopefully) :nervous:
